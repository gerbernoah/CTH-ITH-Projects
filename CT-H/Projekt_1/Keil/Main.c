...
{ 
    unsigned int i;
    led_false = led_true = error = output = 0; // reset everything

    while (dip_submit);         // stay here until code is submited
    selected_index = input >> 4;     //return dip state

    for(i = 0; i < (sizeof(codes[selected_index]) / sizeof(int)); i++)
    {
        while (input_submit)
        {
        ...
        if (output != codes[selected_index][i]) // increment error count if code is wrong
            error = 1;
